...
namespace EthereumPHP\Contracts;

use InvalidArgumentException;
use stdClass;
use EthereumPHP\Utils;
use EthereumPHP\Formatters\IntegerFormatter;
...
class Ethabi
{
    ...
    public function decodeParameters($types, $param)
    {
        ...
        if ($typesLength <= 1) {
            return $solidityTypes[0]->decode($param, $offsets[0], $types[0]);
        }
        
        for ($i = 0; $i < $typesLength; $i++) {
            if (isset($outputTypes['outputs'][$i]['name']) && $outputTypes['outputs'][$i]['name']) {
                $result[$outputTypes['outputs'][$i]['name']] = $solidityTypes[$i]->decode($param, $offsets[$i],
                    $types[$i]);
            } else {
     ...
     *
     * @param string                              $type
     * @param \EthereumPHP\Contracts\SolidityType $solidityType
     * @param mixed                               $encode
     * @param int                                 $offset
     *
     * @return string
    ...
    protected function encodeWithOffset($type, $solidityType, $encoded, $offset)
    {
        if ($solidityType->isDynamicArray($type)) {
            ...
            for ($i = 0; $i < count($encoded); $i++) {
                ...
                $result .= $this->encodeWithOffset($nestedName, $solidityType, $encoded[$i],
